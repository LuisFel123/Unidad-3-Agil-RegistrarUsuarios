<%-- 
    Document   : Medidas
    Created on : 10 nov 2024, 13:07:35
    Author     : felip
--%>
...
<html>
    ...
    <body>
        <%
            ...
            String mensaje = (String) request.getSession().getAttribute("alerta");
        %>
        <%
            ...
            try {
                Double estaturaO = (estatura != null && !estatura.isEmpty()) ? Double.parseDouble(estatura) : 0.0;
                Double pesoO = (peso != null && !peso.isEmpty()) ? Double.parseDouble(peso) : 0.0;
                Double cinturaO = (cintura != null && !cintura.isEmpty()) ? Double.parseDouble(cintura) : 0.0;
                Double caderaO = (cadera != null && !cadera.isEmpty()) ? Double.parseDouble(cadera) : 0.0;

                if (estaturaO != null && pesoO != null && estaturaO != 0.0 && pesoO != 0.0) {
                    IMC = pesoO / (estaturaO * estaturaO);
                } else {
                    System.out.println("Estatura o peso no válidos para calcular IMC.");
                }

                if (cinturaO != null && caderaO != null && cinturaO != 0.0 && caderaO != 0.0) {
                    ICC = cinturaO / caderaO;
                } else {
                    System.out.println("Cintura o cadera no válidos para calcular ICC.");
                }
        ...
        <div
            ...
            <div class="w-full">
                <div class="text-center">
                    <h1 class="text-3xl font-semibold text-gray-900">Agrega tus medidas</h1>
                </div>
                <div class="mt-5">
                    <form action="Calculo" method="post">
                        <div class="relative mt-6">
                            <input type="number" step="any" name="peso" value="" required="true" class="peer mt-1 w-full border-b-2 border-gray-300 px-0 py-1 placeholder:text-transparent focus:border-gray-500 focus:outline-none" autocomplete="NA" />
                            <label for="usuario1" class="pointer-events-none absolute top-0 left-0 origin-left -translate-y-1/2 transform text-sm text-gray-800 opacity-75 transition-all duration-100 ease-in-out peer-placeholder-shown:top-1/2 peer-placeholder-shown:text-base peer-placeholder-shown:text-gray-500 peer-focus:top-0 peer-focus:pl-0 peer-focus:text-sm peer-focus:text-gray-800">Agrega tu peso</label>
                        </div>


                        <div class="relative mt-6">
                            <input  type="number" name="cintura" value="" required="true" class="peer mt-1 w-full border-b-2 border-gray-300 px-0 py-1 placeholder:text-transparent focus:border-gray-500 focus:outline-none" autocomplete="NA" />
                            <label for="usuario1" class="pointer-events-none absolute top-0 left-0 origin-left -translate-y-1/2 transform text-sm text-gray-800 opacity-75 transition-all duration-100 ease-in-out peer-placeholder-shown:top-1/2 peer-placeholder-shown:text-base peer-placeholder-shown:text-gray-500 peer-focus:top-0 peer-focus:pl-0 peer-focus:text-sm peer-focus:text-gray-800">Agrega tu cintura</label>
                        </div>
                        <div class="flex justify-center space-x-4 my-6">
                            <div class="my-6">
                                <button type="submit" value="calcular" action="medidas.jsp" name="calculo"  class="w-full rounded-md bg-black px-3 py-4 text-white focus:bg-gray-600 focus:outline-none">Calcular</button>
                            </div>
                            <div class="my-6">
                            ...
                            <div class="my-6">
                                <button type="submit" value="otro" name="otro" class="w-full rounded-md bg-black px-3 py-4 text-white focus:bg-gray-600 focus:outline-none">Otro</button>
                            </div>
